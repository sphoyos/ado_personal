capture program drop ana_descriptiu_cemcat
program define ana_descriptiu_cemcat
 ...
 local nvar=0 
   ...
   local nomg_`nvgrup'="`nomvargrup'" 
    ...
    if "`lang'"=="cast" { 
        ...
        htput   En primer lugar se presenta un análisis descriptivo para cada una de las variables cualitativas y cuantitativas seleccionadas en función de 
        htput la variable de agrupación `nomvargrup' <br>
		htput </FONT color>
    }   
    ...
    if "`lang'"=="cat" { 
        ...
        htput <FONT color="#993366">
        htput En primer lloc es presenta una anàlisi descriptiva per a cadascuna de les variables de la base de dades en funció de les variables qualitatives
        htput i quantitatives seleccionades en funció de la variable de agrupació <br>
		htput </FONT color>
	}	
	...
	if "`lang'"=="eng" { 
        ...
        htput <FONT color="#993366">
        htput Firstly a descriptive analysis for each qualitative and quantitative variable is presented
        htput by grouping variable<br>
		htput </FONT color>
    }   
...
******** GENERAR TAULES DE 2 X 2 PER A VARIABLES CUALITATIVES ***************************************
       ...
       htput <FONT color="#993366">
       htput Para las variables cualitativas se muestra una tabla de frecuencias con el número de casos y el % de en cada categoria. 
...
******** GENERAR TAULES DE 2 X 2 PER A VARIABLES CUALITATIVES ***************************************
      ...
      if "`txtest_cat'"==""  local txtest_cat=" Chi square test for or Fisher's exact p value when the value of expected cases in each cell is less than 5." 
       ...
       htput <FONT color="#993366">
       htput For qualitative variables, a frequency table shows the number of cases and % in each category.
    ...
    foreach var in   `varcat1' `varcat2'  `varcat3' `varcat4'    {
      ...
      ******************** EFECTUA LA TAULA. CANVIAR PERCENTAGES SEGONS ES VULLGUI    ************
       ...
       htput <BR>
        ...
        local lab_vargrup : value label `vargrup'
	    levelsof `vargrup', local(lev)
   
	   local lab_vargrup : value label `vargrup'
           foreach val of local lev {
        count if `vargrup'==`val' & `var'!=.	
		local cuant=r(N)

		
        local k `:label `lab_vargrup' `val''
		 local cn: disp  "`k'(`cuant') "
        
        label define lab_new `val'  "`cn'" , modify
		}
		label val `vargrup' lab_new
	...
	if "`grafcol'"=="" { 		
            ************************ DIBUIXA EL GRAFIC DE FREQÜENCIES ACUMULAT *****************
            catplot bar `var'  `vargrup', percent("`vargrup'") stack asyvars title ("`nomvar'")   ///
                  				blabel(bar, position(inside) color(gs2) size(large)  format(%3.1f)) 	 /// 								
                       bar(5, bcolor( 4 107 117))      bar(4, bcolor(0 164 163)) bar(3, bcolor(121 213 213))  bar(2,bcolor(138 252 252))  bar(1, bcolor(210 255 255))  ///
		    	    oversubopts(label(labsize(small))) ylabel( ,labsize(small) angle(h))ytitle("`ytit1'" ,size(small) ) ///
					 note( Information available for `n_`nvgrup'_`nvar'')  legend(title( "`nomvar'",size(small)) cols(3) bmargin(zero) size(small) region(lstyle(none))) 
          }
		  else {
		    ************************ DIBUIXA EL GRAFIC DE FREQÜENCIES ACUMULAT *****************
            catplot bar `vargrup' `var', percent("`var'") stack asyvars title ("`nomvar'")   ///
                       				blabel(bar, position(inside) color(gs2) size(large)  format(%3.1f))	 /// 								
              bar(5, bcolor( 7 107 117))      bar(4, bcolor(0 164 163)) bar(3, bcolor(121 213 213))  bar(2,bcolor(138 252 252))  )  bar(1, bcolor(210 255 255))  ///
		      oversubopts(label(labsize(small))) ylabel( ,labsize(small) angle(h) )ytitle("`ytit1'" ,size(small) ) note(`note1'`pvalue') ///
			  legend(title( "`nomvargrup'",size(small)) cols(3) bmargin(zero) size(small) nobox) 
          }
...
******** GENERAR TAULES DESCRIPTIVES  PER A VARIABLES QUANTITATIVES ***************************************
    ...
    ******************** ACTIVA  EL CATALA O CASTELLA *****************************************
	...
	if `ncatgrup'==2 & "`txttest_cuant'"=="" {
        htput   Se presentan los p valores de dos pruebas de contraste de igualdad de las variables en los grupos:
	    htput La prueba t y la prueba no paramètrica de Mann-Whitney.   
	    htput Se elegirà el valor p de una u otra según la simetria de la distribución o la prueba de normalidad  y si el tamaño es suficiente.
    } 
    ...
    htput  En el diagrama de cajas que se presenta después de cada tabla . La linea de la caja representa la mediana 
    ...
    ******************** ACTIVA  EL CATALA O CASTELLA *****************************************
    ...
    if `ncatgrup '== 2 & "`txttest_cuant'" == "" {
        htput Es presenten els p valors de dues proves de contrast d'igualtat de les variables en els grups:
        htput La prova t i la prova no paramètrica de Mann-Whitney.
        htput Es triarà el valor p d'una o altra segons la simetria de la distribució o la prova de normalitat i si la mida és suficient.
    }                                            
    else if `ncatgrup '> 2 & "`txttest_cuant'" == "" {
        htput Es presenten els p valors de dues proves de contrast d'igualtat de les variables en els grups:
        ...
        htput En el diagrama de caixes que es presenta després de cada taula. La línia de la caixa representa la mitjana
    ...
    ******************** ACTIVA  EL CATALA O CASTELLA *****************************************
    ...
    htput <BR>
    htput <FONT color = "#993366">
    htput For quantitative variables usual descriptive variables are calculated, mean, standard deviation, minimum and maximum and median 
    htput and percentiles according to grouping variable.
	
    if `ncatgrup'== 2 & "`txttest_cuant'" == "" {
        htput P values of two hypothesis test for equality of distribution variables in the groups are presented:
        htput The t test and the nonparametric Mann-Whitney test.
        htput The p value will be chosen according to the symmetry of the distribution or normality test and if the  sample size is large enough
    }
    else if `ncatgrup'> 2 & "`txttest_cuant'" == "" {
        htput P values of two hypothesis test for equality of distribution variables in the groups are presented:
        ...
        htput The box plot is presented after each table. The line of the box represents the median
...
***** FA UN BUCLE  PER A CADASCUNA DE LES VARIABLES QUANTITATIVES ***************
   ...
   local n_`nvgrup'_`nvar'=r(N)
     if  `robs'> 1 {
       ht_tablacont_`lang' `var' `vargrup' , head close `cuant_options' `coloret'
...
local lab_vargrup : value label `vargrup'
        ...
        ************************* DIBUIXA ELS DIAGRAMES DE CAPSES *****************
        ...
        graph box `var'  , over(`vargrup') title ("`nomvar'" )  caption (" `nomvar'") ytitle(" ")  ylabel(, labsize(small) format(`formvar') angle(h)) ///
  box(1, fcolor(121 213 213) lcolor(7 107 117)) marker(1, mcolor(7 107 117))	 
           label val `vargrup' `lab_vargrup'
	*********** GUARDA ELS GRAFICS ******************************************
     graph export $htm\png\gr_`grafname'.png, replace
     graph export $gph\wmf\gr_`grafname'.wmf, replace
    ...
    ***************** TANCA EL BUCLE DELS GRAFICS    
	 ...
	 if `grafics_cuant'==1 {
        ...
        if   `"`nomvar'"' == "" local nomvar = "`var'"
	     ...
	     local formvar: format `var' 
        graph box `var',   title ("`nomvar'" )    ytitle(" ")  ylabel(, labsize(small) format(`formvar') angle(h)) ///
        box(1, fcolor(121 213 213) lcolor(7 107 117)) marker(1, mcolor(7 107 117))		
...
if "`lang'"=="cat" { 
     ...
     htput A la següent taula es presenta un resum de les associacions entre les variables considerades
...
if "`lang'"=="cast" { 
     ...
     htput <BR>
     htput <FONT color="#993366" size="4" >  
     htput En la siguiente tabla se presenta un resumen de las asociaciones entre las variables consideradas
...
if "`lang'"=="eng" { 
     ...
     htput <BR>
     htput <FONT color="#993366" size="4" >  
     htput The following table summarizes the associations between  considered variables 
...
if round(`p_`nvgrup'_`nvar'',0.001) <=`psig' & "`p_`nvgrup'_`nvar''"!="" &  {	
...
if round(`p_`nvgrup'_`nvar'',0.001) <=`psig' {	
	                local coloret= `"BGCOLOR="#FFCCCC""' 
	               		    }
		       else	{
                    local coloret=""
