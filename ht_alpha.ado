*! version 4.8.9  04may2018
*! version 4.8.9  31jul2013
*! Version for Html output of 
program define ht_alpha, rclass byable(recall)
        version 8, missing

        * undocumented: coo == casewise
        syntax varlist(min=2 numeric) [if] [in] [, Asis Casewise Coo Detail Item /*
         */ Generate(string)  Min(integer 1) Reverse(varlist numeric) Std ]
        ...
        */
        ...
        if "`item'" == "" {
                ...
                if "`revlist'" != "" {
						...
						htput <p> Reversed `itemtxt':  <b> `revlist' </b> </p>
				...
				local res1:di `fmt' `acorr'
				local res2:di  %9.0f `k'
				local res3: di %9.4f `alpha'
				
				
				htput <TABLE BORDER=1 CELLSPACING =0 CELlPADDING=2>
					htput <TR>
						htput <TH>Average interitem `lab':  </TH>
						htput  <TD> `res1' </TD>
					htput </TR>
					htput <TR>
						htput <TH>Number of items in the scale: </TH>
						htput  <TD> `res2' </TD>
					htput </TR>
					htput <TR>
						htput <TH>Scale reliability coefficient: </TH>
						htput  <TD> `res3' </TD>
					htput </TR>
				htput </TABLE>	
        ...
        if "`item'" != "" {
				...
				htput <TABLE BORDER=1 CELLSPACING =0 CELlPADDING=2>
					htput <TR>
						...
						htput  <TH>Obs </TH>
						htput  <TH>Sign </TH>
						htput  <TH>Item-test <br> Correlation </TH>
						htput  <TH>Item-rest <br> Correlation  </TH>
						htput  <TH>Average <br>interitem<br> `lab' </TH>
						htput  <TH>Cronbach's Alpha </TH>
					htput </TR>
                ...
                forvalues i = 1/`k' {
						...
						local res1: var label ``i''
					    if  `"`res1'"' == "" local res1 = "``i''"
				   		local res2: di %4.0f `Obs'[`i',`i']
						local res3 : di   =cond("`sgn`i''"=="1","+","-")
						local res4: di  %7.4f `ITC'[1,`i'] 
						local res5: di  %7.4f `IRC'[1,`i'] 
						local res6: di `fmt' `mIIC'[1,`i'] 
						local res7: di %7.4f `alph'[1,`i']
						
						  htput <TR>
								htput <TD> <b>`res1'</b> </TD>
								htput <TD> `res2' </TD>
								htput <TD> `res3' </TD>
								htput <TD> `res4' </TD>
								htput <TD> `res5' </TD>
                ...
                di as txt "{hline 13}{c BT}{hline 65}"
						...
						local res6:di `fmt' `acorr' 
						local res7: di %7.4f `alpha'
				 
					  htput <TR>
								htput <TD> <b>Test scale</b> </TD>
								htput <TD COLSPAN=4 >  </TD>
								htput <TD> `res6' </TD>
								htput <TD> `res7' </TD>
						 htput </TR>	
        }
        ...
        * --------------------------------------------------------------------------
        ...
        if "`detail'" != "" {
                ...
                if "`std'" != "" {
						...
						htput <p> <b> Interitem correlations `rev'`obs' </b></p>
                        htlog mat list `C', noheader format(%6.4f)
                }
                else {
                        di as txt _n "Interitem covariances `rev'`obs'"
						htput <p> <b>Interitem covariances `rev'`obs'</b></p>
                        htlog mat list `C', noheader format(%9.4f)
                }

                if `N' != `NC' {
                        di as txt _n "Pairwise number of observations"
						htput <p><b>Pairwise number of observations </b></p>
                        mat rownames `Obs' = `varlist'
                        mat colnames `Obs' = `varlist'
                        htlog  mat list `Obs', noheader format(%5.0f)
                }
        }
