...
cap prog drop ht_baplot
prog define ht_baplot
version 6.0
...
*/ text(real 100) ratio(real 1) *]"
	...
	local labm1:var label `m1'
	if   `"`labm1'"' == "" {
		local labm1 = "`m1'"
		}
	local labm2:var label `m2'
	if   `"`labm2'"' == ""  {
		local labm2 = "`m2'"
		}
	...
	#delim cr
	 ...
	 htput <TABLE BORDER=1 CELLSPACING =0 CELlPADDING=2>
      ...
      htput <TR>
      htput <TH VALIGN=CENTER ALIGN=CENTER COLSPAN =2  > Bland-Altman comparison of `m1' and `m2' </TH>
      htput </TR>
 
	   htput <TR>
	   local p1:di  "Limits of agreement (Reference Range for difference) " 
       local p2: di  	  `format' `lrr'  " to "  `format' `urr'   
       htput <TD VALIGN=CENTER  > `p1' </TDd>
       htput <TD VALIGN=CENTER ALIGN=CENTER   > `p2' </TD>
       htput </TR>
 	
		   htput <TR>
	   local p1:di "Mean difference "
       local p2: di `format' `xbar'  " (CI "  `format' `lcb'  " to "  `format' `ucb'  ") "
       htput <TD VALIGN=CENTER  > `p1' </TDd>
       htput <TD VALIGN=CENTER ALIGN=CENTER   > `p2' </TD>
       htput </TR>
	   ...
	   htput <TR>
	   local p1:di "Range  "
       local p2: di `format' `xmin' " to "  `format' `xmax'
       htput <TD VALIGN=CENTER  > `p1' </TDd>
       htput <TD VALIGN=CENTER ALIGN=CENTER   > `p2' </TD>
       htput </TR>
	   ...
	   htput <TR>
	   local p1:di "Pitman's Test of difference in variance  "
       local p2: di "r = "  `format' `r'  ", n = "  `n'  ", p ="  `format' `sig'
       htput <TD VALIGN=CENTER  > `p1' </TD>
       htput <TD VALIGN=CENTER ALIGN=CENTER   > `p2' </TD>
       htput </TR>
