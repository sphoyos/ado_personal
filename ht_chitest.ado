capture program drop ht_chitest
*! 2.0.1 NJC 10 March 2004 
*! 2.0.0 NJC 10 July 2003 
program ht_chitest, sort rclass 
	version 8
	...
	if `nvars' == 2 { 
		...
		local text: di  "observed frequencies "   cond("`count'" != "", "of ", "from ")  ///
   		cond("`kb'" == "kb", "keyboard", "`1'") "; "   ///
    ...
    local text: di "likelihood-ratio chi2("  "`df'"               ///
		 ") = "  %8.4f `chi2_lr'  "   Pr = " /// 
		 %6.3f `p_lr'
	...
	label var `exp' "Expected"
    label var 	`res' "Obs-Exp"
	label var `Pearson' "Pearson"
	
	
	
	htlist `counted' `obs' `exp' `notes' `res' `Pearson' if `touse',  noobs  `options' 
