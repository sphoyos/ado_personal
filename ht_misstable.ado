*! version 1.5.3  27jun2011
program ht_misstable
	version 11

	gettoken cmd 0 : 0, parse(" ,")
	local l = strlen("`cmd'")
     
	if ("`cmd'"==substr("nested", 1, max(4,`l'))) {
		misstable_nested `0'
	}
	else if ("`cmd'"==substr("patterns", 1, max(3,`l'))) {
		misstable_patterns `0'
	}
	else if ("`cmd'"==substr("summarize", 1, max(3,`l'))) {
...
program misstable_summarize, rclass
	...
	*"************************************************
	***************  GENERA TAULA SUMMARIZE *********
	*************************************************
		htput <TABLE BORDER=1 CELLSPACING =0 CELlPADDING=2>
	  
	  htput <TR>
      htput  <TH VALIGN=CENTER ALIGN=CENTER   >   </TH> 
	  htput  <TH VALIGN=CENTER ALIGN=CENTER COLSPAN=3  > Frequència tipo observacions  </TH>
      htput   <TH VALIGN=CENTER ALIGN=CENTER COLSPAN=3  > Obsservacions amb dades  </TH>	  
	  htput </TR>
	  htput <TR>
      htput  <TH VALIGN=CENTER ALIGN=CENTER   > Variable  </TH> 
	  htput  <TH VALIGN=CENTER ALIGN=CENTER  > Perdudes (obs==.)  </TH>
	  htput  <TH VALIGN=CENTER ALIGN=CENTER  > Perdudes (obs>=.)  </TH>
	  htput  <TH VALIGN=CENTER ALIGN=CENTER  >Amb dades (obs<.)  </TH>
      htput   <TH VALIGN=CENTER ALIGN=CENTER  > Valors únics  </TH>	  
      htput   <TH VALIGN=CENTER ALIGN=CENTER  > Mínim  </TH>	  
	  htput   <TH VALIGN=CENTER ALIGN=CENTER  > Màxim  </TH>	
	  htput </TR>
	...
	foreach v of local varlist {
	 local nomvar:var label `v'
     if `"`nomvar'"' =="" local nomvar="`v'"
		...
		if (substr("`ty'",1,3)!="str") { 
			...
			quietly {
				...
				if ("`genstub'"!="") {
if (length("`genstub'`v'")>`c(namelen)') {
	local genvname = abbrev("`v'", `c(namelen)'-length("`genstub'"))
	local genvname = strtoname("`genvname'")
...
label variable `genvname' "(`v'`geneq')" 
	  ...
	  htput <TR>
      htput  <TD VALIGN=CENTER ALIGN=RIGHT   > `nomvar'  </TD> 
	  
			di as smcl as txt _skip(`indent') "`vname' {c |}" _c
			if (return(N_eq_dot) | `showzeros') {
				di as smcl as res %10.0gc return(N_eq_dot) _c
				local txt: di  %10.0gc return(N_eq_dot)
				htput  <TD VALIGN=CENTER ALIGN=CENTER   > `txt'  </TD> 
			}
			else {
			...
			if (return(N_gt_dot) | `showzeros') {
				...
				local txt: disp   %10.0gc return(N_gt_dot)
				htput  <TD VALIGN=CENTER ALIGN=CENTER   > `txt'  </TD>
			}
			else {
				di as smcl as txt "{space 10}" _c
            ...
            local txt: di   %10.0gc return(N_lt_dot)
            htput  <TD VALIGN=CENTER ALIGN=CENTER   > `txt'  </TD>
			if (return(K_uniq)>=.) { 
					 	/*1234567890*/
				...
				local txt :di  %7.0gc return(K_uniq)
				htput  <TD VALIGN=CENTER ALIGN=CENTER   > `txt'  </TD>
			}

			di as smcl as txt "  "  %9.0g return(min)	///
							  	  "   " %9.0g return(max)
				 local txt: disp  %9.0g  return(min)	
                 htput  <TD VALIGN=CENTER ALIGN=CENTER   > `txt'  </TD>	
				 local txt: disp  %9.0g  return(max)	
                 htput  <TD VALIGN=CENTER ALIGN=CENTER   > `txt'  </TD>				
		 htput </TR>						  
		}
		else {
			...
			" (string variable)" _col(49) "{c |}"
				  ...
				  htput <TR>
            htput  <TD VALIGN=CENTER ALIGN=CENTER   > `nomvar'  </TD> 
			htput  <TD VALIGN=CENTER ALIGN=CENTER  colspan=6 > string variable  </TD> 
			htput </TR>
		}
