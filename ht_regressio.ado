...
*! Version 5.0 Aug 2013 by SPH,general  regressio with constant, no permutation 
...
program define ht_regressio
version 9.1
syntax [if] [in], reg(string) [dep(varname)] indep(string) [rr(string)] [trans(string)] [per(string)] [multi] [PERMute(integer 0)] [color] [options(string)] [adjust(string)] [note(string)]  [constant]
...
file open prg_code using `_mymodel'.ado, write replace
	...
	file write prg_code "version 11.1:xi: `""cmd""' `""dep""' `""indep""' `""adj""' `""if""' `""in""', `""opt""'" _n
...
if "`reg'"!="stcox"  & "`reg'"!="stcrreg" {
    ...
    local analisi "Regressio `reg' per a `deplab'" 
        if  ( "`multi'"!=""  | wordcount("`indep'")==1 ) & "`constant'"!="" {
             local tab_close=""
             }
        else {
            local tab_close="close"
        }
}
if "`reg'" =="stcox" | "`reg'" =="stcrreg"  {
local opt= "`opt'" + " nohr"
local analisi "Regressio `reg' " 
local tab_close="close"
}
htput <BR>
...
if "`1'"!="" {
	...
	* model estimation	
	if "`1'"=="`v1'" {
		* continuous variable
		...
		quietly version 11.1:xi:`reg' `dep' `1' `adjust' `if' `in', `options'
	...
	* making-up the table
	if "`2'"!="" {
		...
		* no more variables
		if "`1'"=="`v1'" {
		* continuous variable
		htputvar `v1', head rr(`rr') define coef(`1') strata(1) trans(`trans') pval(`p') per(`per') `tab_close' `perm_results' `color'
		}
		else {
		* categorical variable
		...
		local c="_I`sub'_`val'"
		local val= `v1'[`cat']	
		htputvar `v1', strata(`cat') coef("`c'") val(`val') trans(`trans') `color' `tab_close' 
		restore
		}
	 } 
  }
  if   "`tab_close'"!="close"  {	
		   ...
		   local intercept: di %6.3f (_b[_cons])
		   local li_interc: di  %6.3f (_b[_cons]) - invnorm(0.975)* _se[_cons] 
		   local ls_interc: di  %6.3f (_b[_cons]) + invnorm(0.975)* _se[_cons]
		    htput  <TR>
	  	   htput  <TD  VALING=TOP ROWSPAN= 1 COLSPAN= 2> Intercepto<sup>*</sup></TD> 
		   htput <TD ALIGN=RIGHT>  `intercept' </TD>
	       htput <TD ALIGN=CENTER > (`li_interc';`ls_interc') </TD>
		   htput <TD ALIGN=CENTER >  </TD>
		   htput  </TR>
		   htput  </TABLE>	
		   htput <SMALL> Corresponde al valor de la constante cuando todas las variables son cero  </SMALL>
		   htsupwri
...
while "``i''"!="" {
	...
	* model estimation	
	if "``i''"=="`v`i''" {
		* continuous variable
		quietly version 11.1:xi:`reg' `dep' ``i'' `adjust' `if' `in', `options'
		noi	disp in yellow "`1'"
	noi di  in yellow %5.2f _b[``i'']
	noi di  in yellow %5.2f _se[``i'']
	...
	* making-up the table
	if "``j''"!="" {
		...
		* no more variables
		if "``i''"=="`v`i''" {
		* continuous variable
		htputvar `v`i'', define coef(``i'') trans(`trans') strata(1) pval(`p') per(`per') `tab_close' `perm_results' `color'
		}
		else {
		* categorical variable
		...
		local c="_I`sub'_`val'"
		local val= `v`i''[`cat']	
		htputvar `v`i'', strata(`cat') coef("`c'") val(`val') trans(`trans') `color' `tab_close'
		restore
		}
	}
...
if "`1'"!="" {
	...
	* model estimation	
	if "`1'"=="`v1'" {
		* continuous variable
		quietly version 11.1:xi:`reg' `dep' `indep' `adjust' `if' `in', `options'
		count if e(sample)
		global HTmitot `r(N)'
	...
	* making-up the table
	if "`2'"!="" {
		...
		* continuous variable
		...
		htputvar `v1', head rr(`rr') define coef(`1') trans(`trans') strata(1) pval(`p') per(`per') `tab_close' `perm_results' `color'
		}
		else {
		* categorical variable
...
if   "`tab_close'"!="close"  {	
		   ...
		   local ls_interc: di  %6.3f (_b[_cons]) + invnorm(0.975)* _se[_cons]
		    htput  <TR>
	  	   htput  <TD  VALING=TOP ROWSPAN= 1 COLSPAN= 2> Intercepto </TD> 
		   htput <TD ALIGN=RIGHT>  `intercept' </TD>
	       htput <TD ALIGN=CENTER > (`li_interc';`ls_interc') </TD>
		   htput <TD ALIGN=CENTER >  </TD>
		   htput  </TR>
