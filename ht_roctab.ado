cap drop ht_roctab.ado
*! version 7.3.8  11feb2007
prog def ht_roctab, rclass sortpreserve
	version 7, missing
	if _caller() < 8 {
		roctab_7 `0'
		return add
...
senspec `D' `K' , sensitivity(`sensy') specificity(`specy')



gen `vyouden'= `sensy' -(1-`specy')
qui summarize(`vyouden') if  `vyouden'!=.
local youden=r(max)
disp "`youden'"
qui summarize(`K') if round(`vyouden',0.0000001)==round(`youden',0.0000001)
local cutoffy=r(max) 
qui summarize(`sensy') if round(`vyouden',0.0000001)==round(`youden',0.0000001) 
local sensiy=r(max) 
qui summarize(`specy') if round(`vyouden',0.0000001)==round(`youden',0.0000001)
local speciy=r(max) 

disp in yellow "**************** `sensiy'; `speciy'"

gen `vdist' = sqrt((1-`sensy' )^2 + (1-`specy')^2)
qui summarize(`vdist')
qui local dist=r(min)
summarize(`K') if `vdist'==r(min)
local cutoffd=r(max)
...
nobreak {
		...
		MYDIspl `N' `D' `wt' `touse' `area' `se' `level' `youden' `sensiy' `speciy' `cutoffy' `cutoffd' `mygr' `SEtype' `CItype'  
		...
		return scalar sensi=`sensiy'
		return scalar speci=`speciy'
		return scalar cuty=`cutoffy'
		return scalar cutd=`cutoffd'
...
prog def MYDETail
	...
	*/ "   Classified          LR+          LR-"
	noi di in smcl in gr "{hline 78}"
	  ...
	  htput <BR>
	  htput <p> <b><font color="#993366"  >  Detailed report of Sensitivity and Specificity </b>  </font> </p>

	  htput <TABLE BORDER=1 CELLSPACING =0 CELlPADDING=2>
   htput <TR>
      htput <TH VALIGN=CENTER ALIGN=LEFT > CUTPOINT </TH>
      htput <TH VALIGN=CENTER ALIGN=LEFT > SENSITIVITY </TH>
	  htput <TH VALIGN=CENTER ALIGN=LEFT > SPECIFICITY </TH>
	  * htput <TH VALIGN=CENTER ALIGN=LEFT > 1-SPECIFICITY </TH>
	  htput <TH VALIGN=CENTER ALIGN=LEFT > CORRECTED CLASSIFIED </TH>
	  htput <TH VALIGN=CENTER ALIGN=LEFT > LR+ </TH>
	  htput <TH VALIGN=CENTER ALIGN=LEFT > LR- </TH>
	  	  htput <TH VALIGN=CENTER ALIGN=LEFT > Youden Index </TH>
   htput </TR>
	...
	while `i'<= `max' + 1 {
		...
		*/ _col(71) %8.4f `lrn' 
		...
		local o_sens: di %8.2f `sens'[`i'] `"%"'
		local o_spec: di %8.2f `spec'[`i'] `"%"'
		local o_1spec: di %8.2f 100-`spec'[`i'] `"%"'
		local o_corr: di  %8.2f `corr' `"%"'
		local  o_lrp: di %8.4f `lrp' 
		local o_lrn:di %8.4f `lrn' 
		local o_youden:di  %8.4f    (`sens'[`i'] -( 100-`spec'[`i'] ))/100 
		
	  htput <TR>
      htput <TD VALIGN=CENTER ALIGN=LEFT > `cut' </TH>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `o_sens' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `o_spec'  </TD>
	 * htput <TD VALIGN=CENTER ALIGN=CENTER > `o_1spec'  </TD>
      htput <TD VALIGN=CENTER ALIGN=CENTER > `o_corr'   </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER >  `o_lrp'  </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `o_lrn' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `o_youden' </TD>
	  htput </TR>
	  local i= `i'+1
...
prog def MYDIspl, sclass
	args N  D wt touse  area nse level youden sensiy speciy cutoffy cutoffd graph SEtype CItype  
...
*/ `"`spaces'[`=strsubdp("`level'")'% Conf. Interval]"' _n "         " /*
	...
	local citype =subinstr("`citype'","{hline 1}","",.)
    local citype =subinstr("`citype'","{hline 2}","",.)
	  ...
	  htput <TABLE BORDER=1 CELLSPACING =0 CELlPADDING=2>
      htput <TR>
      htput <TH VALIGN=CENTER ALIGN=CENTER > Observaciones </TH>
	  htput <TH VALIGN=CENTER ALIGN=CENTER > ROC AREA </TH>
	  htput <TH VALIGN=CENTER ALIGN=CENTER> `SEtype' Std. Error </TH>
	  local citext: disp `"`spaces'[`=strsubdp("`level'")'% Conf. Interval]"'
	   htput <TH VALIGN=CENTER ALIGN=CENTER >`citype'   `citext' </TH>
	   htput <TH VALIGN=CENTER ALIGN=CENTER >Indice Youden </TH>
	   htput <TH VALIGN=CENTER ALIGN=CENTER >Sensibilidad según  Youden </TH>
	   htput <TH VALIGN=CENTER ALIGN=CENTER >Especificidad según Youden </TH>
	   htput <TH VALIGN=CENTER ALIGN=CENTER >Classificados correctamente según Youden </TH>
	   htput <TH VALIGN=CENTER ALIGN=CENTER > Punto corte según Youden</TH>
	   htput <TH VALIGN=CENTER ALIGN=CENTER >Mejor Punto corte AUC </TH>
	  ...
	  local corry=  (`sensiy'*`N1' + `speciy'*`N0') /(`N1'+`N0')
	  
	  
	  local t_obs :di  %8.0f `N'
	  local t_area: di %8.4f `area'
	  local t_nse : di %8.4f `nse' 
	  local t_lb: di %8.4f `lb' 
	   local t_ub: di %8.4f `ub'
	   local t_sens: di %8.4f `sensiy'*100
	   local t_spec: di %8.4f `speciy'*100
	    local t_corry: di  %8.4f `corry'*100
	   local t_youden: di  %8.4f `youden'
	   local t_cutoffy: di  %8.2f `cutoffy'
	   local t_cutoffd: di  %8.2f `cutoffd'
	  htput <TR>
      htput <TD VALIGN=CENTER ALIGN=CENTER > `t_obs' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_area' </TD>
	   htput <TD VALIGN=CENTER ALIGN=CENTER > `t_nse' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_lb' ; `t_ub' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_youden' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_sens' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_spec' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_corry' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_cutoffy' </TD>
	  htput <TD VALIGN=CENTER ALIGN=CENTER > `t_cutoffd' </TD>
	  htput </TR>
	  htput </TABLE>
	  htput <br>
