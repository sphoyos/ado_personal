*!version 6.4.2  02aug2009
program define ht_stphtest, rclass sort
version 6
	if _caller() < 8 {
		stphtest_7 `0'
		return add
...
*/
	if "`plot'"!="" {
		if "`detail'"!="" {
			di in red "may not specify both plot and detail"
			exit 198
		}
	...
	if `nopts'>1 { 
		noi di in red /*
		*/ "may only specify one of km, log, plot, rank, and time()"
		exit 198
	}
	...
	if "`e(prefix)'" == "svy" {
		di as err "stphtest not allowed after estimation with svy"
		exit 322
	}
...
if "`useoldbase'" != "" {
		...
		if "`e(vl_ssc)'" == ""  {
            		noi di in red  /*
*/ "stcox did not save scaled Schoenfeld residuals." _n /*
*/ "scaledsch(newvars) option must have been specified on the stcox command" /*
*/ _n "prior to using the detail and plot options with this command"
			exit 198
		}
       	}
	...
	local flag  0
		...
		if "`detail'"!="" {
			noi di in gr /*
*/ "note: cannot perform global test because schoenfeld(newvars) option was" /*
			*/ _n _col(7) "not specified when stcox was estimated"
		local flag 1
		}
		else {
			noi di in red /*
*/ "cannot perform global test because schoenfeld(newvars) option was" /*
			*/ _n _col(7) "not specified when stcox was estimated"
			exit 198
		}
	}
...
if "`plot'"=="" {
    ...
    htput  <font color="#8B57B8" > Test de riesgos proporcionales (Therneau-Grambsch) </font color>
	noi di in gr  "      Time:  " in ye "`title'"
	htput  <font color="#8B57B8" > Tiempo:  <b>  `title' </b> </font color>
	...
	if "`plot'"!="" | "`detail'"!="" {
		...
		if "`plot'"!="" {
			...
			while `i'<=`n' { 
				...
				local new_lbl: variable label `lbl'
			    if   "`new_lbl'"' == "" {
				
				local new_lbl = "`lbl'"
                }			
					
					local new_lbl=trim("`new_lbl'")
				noi di in smcl in  gr "      "  /*
					*/ abbrev("`lbl'",12) /*
					*/ _col(19) "{c |}" /* 
				...
				local RHO : di %8.5f `rho'
				local CHI2: di  %9.2f `chi2'
				local DF: di %5.0f 1 
				local PVAL: di %5.4f chiprob(1,`chi2')
			    htput <TR> 				
		        htput <TD> `new_lbl'</TD>
		        htput <TD> `RHO' </TD>
		        htput <TD> `CHI2' </TD>
	...
	*/  _col(64) %5.4f chiprob(`df', return(chi2))
        noi di in smcl in gr "      {hline 12}{c BT}{hline 51}"
				...
				local CHI2: di  %9.2f return(chi2) 
				local DF: di %5.0f `df'
				local PVAL: di %5.4f chiprob(`df', return(chi2))
			    htput <TR> 				
		        htput <TD> <b>Test global </b> </TD>
		        htput <TD>   </TD>
		        htput <TD> <b>`CHI2'</b> </TD>
		        htput <TD> <b>`DF'</b> </TD>
		        htput <TD> <b>`PVAL'</b></TD>
	            htput </TR>
