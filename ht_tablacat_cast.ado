capture program drop ht_tablacat_cast 
program define ht_tablacat_cast,rclass
...
/* CALCULA ELS PERCENTATGES PER COLUMNA SI SE DEMANEN ELS TOTALS PER COLUMNA */ 
 ...
 version 13.1
  ...
  quietly cap tab `row' `col' , exact `missing'
   if _rc !=910{
       ...
       local method Prueba exacta de Fisher 
	   }
	else {
     	 quietly tab `row' `col' , chi `missing'
          local pval: di %5.3f r(p) 
 ...
 version 13.1
  ...
  quietly cap tab `row' `col' , exact `missing'
   if _rc !=910{
       ...
       local method Prueba exacta de Fisher unilateral
	   }
	else {
     	 quietly tab `row' `col' , chi `missing'
          local pval: di %5.3f r(p) 
 ...
 if "`symexact'"!="" {
  ...
  local pval: di %5.3f r(p_exact)
  local method  test de simetria exacto
 }
