capture program drop ht_tablacat_cat 
program define ht_tablacat_cat ,rclass
...
/* CALCULA ELS PERCENTATGES PER COLUMNA SI SE DEMANEN ELS TOTALS PER COLUMNA */ 
 ...
 version 13.1
  ...
  quietly cap tab `row' `col' , exact `missing'
   if _rc !=910{
       ...
       local method Prova exacta de Fisher 
	   }
	else {
     	 quietly tab `row' `col' , chi `missing'
          local pval: di %5.3f r(p) 
 ...
 version 13.1
  ...
  quietly cap tab `row' `col' , exact `missing'
   if _rc !=910{
       ...
       local method Prova exacta de Fisher unilateral
	   }
	else {
     	 quietly tab `row' `col' , chi `missing'
          local pval: di %5.3f r(p) 
 ...
 if "`symexact'"!="" {
  ...
  local pval: di %5.3f r(p_exact)
  local method  test de simetria exacte
 }
 ...
 if "`power'"!="" {
   ...
   local potencia:disp "Potencia `var_row_lab' vs `var_col_lab'= "  %5.3f 1-nchi2(  (r(c)-1)*(r(r)-1) ,r(chi2_lr),invchi2((r(c)-1)*(r(r)-1),1-0.05))
  ...
  htput <TD ALIGN=left >  `etiq' </TD>
   ...
   local j=`j' +1
	...
	htput  <TD ALIGN=center `coloret_`i'_`j''>  `cell_`i'_`j''   </TD>
	 ...
	 if `total_fila' ==1  {
       htput <TD ALIGN=center  >   `cell_`i'_ncol'  </TD>
   }
