capture program drop ht_tablacat_eng 
program define ht_tablacat_eng,rclass
...
*/					                            [CHI] [EXACT]  [CI] [CIEXACT] [SYMMETRY] [SYMEXACT]  [SKILMACK]  /*
*/                                      [Pval(real -1)]  [POSTHOC]  [EXPECTED] [EXACT1] /*
*/                                      [MIssing] [Power] [Freqsort] /*
*/					[color]
...
if "`prow'" !="" &  "`pcol'" !="" {
 ...
 if "`chi'" !="" & "`exact'" !=""   & "`exact1'" !="" & "`symmetry'" !="" & "`symexact'" !=""  & "`skilmack'" !="" {
...
cap drop `freq'  `tag'  `order'
bysort `row': gen `freq'=_N
egen `tag'=tag(`row')

gsort -`tag'  -`freq'
local frowlevels=" "
   forvalues k=1(1)`nrow' {
     local lev=`row'[`k']
     local frowlevels="`frowlevels'"+ " `lev'"
     local lev=""
   }
 local rowlevels="`frowlevels'"  
}

 
local rowlevels="`rowlevels'   `missing'  "/* missing incluye los casos perdidos como un nuevo valor */ 
disp in yellow "`rowlevels'"
local nrow= wordcount("`rowlevels'" )


local var_col_lab: variable label `col'
if `"`var_col_lab'"'  ==""  & "`col'"!="total" {
...
version 11.1		
		local pct_row: di %5.2f (`num'/`denom_row')*100
			if "`ciexact'"!=""{
			  cii `denom_row' `num' ,binomial exact
			  local ls_pct_row: disp  %5.2f r(ub)*100
			  local li_pct_row: disp  %5.2f r(lb)*100
			}
			else {
			...
			if "`ciexact'"!=""{
			  cii `denom_col' `num' ,binomial exact
			  local ls_pct_col: disp  %5.2f r(ub)*100
			  local li_pct_col: disp  %5.2f r(lb)*100
			}
			else {
...
/* CALCULA ELS PERCENTATGES PER COLUMNA */ 
			 if "`pcol'"!="" {
				  ...
				  local variable: di "N (% column)"
			 
			 
			    if "`ci'"!="" {
                  local  cell_`i'_`j' : di   `num'  " (" `pct_col' "%)" " <p> [ " `li_pct_col' "% ; "  `ls_pct_col'  "% ] </p> " 
				  local variable: di "N (% column) <p> [ IC 95% ] </p> "           
				}
			 }
			 }
}
...
/* CALCULA ELS PERCENTATGES PER COLUMNA SI SE DEMANEN ELS TOTALS PER COLUMNA */ 
	...
	local j=0
	    foreach c in `collevels'{
		   ...
		   local ls_pct_col: di %5.2f invbinomial(`n_total',`num_col',(1-.025)) *100
		   	...
		   	if "`ciexact'"!=""{
			  cii `n_total' `num_col' ,binomial exact
			  local ls_pct_col: disp  %5.2f r(ub)*100
			  local li_pct_col: disp  %5.2f r(lb)*100
			}
			else {
 ...
 version 13.1
  ...
  quietly cap tab `row' `col' , exact `missing'
   if _rc !=910{
       ...
       local method Fisher Exact test
	   }
	else {
     	 quietly tab `row' `col' , chi `missing'
          local pval: di %5.3f r(p) 
 ...
 version 13.1
  ...
  quietly cap tab `row' `col' , exact `missing'
   if _rc !=910{
       local pval: di %5.3f r(p1_exact)
       local method one sided Fisher Exact test
	   }
	else {
     	 quietly tab `row' `col' , chi `missing'
          local pval: di %5.3f r(p) 
 ...
 if "`symexact'"!="" {
  ...
  local pval: di %5.3f r(p_exact)
  local method  Symmetry exact test
 }
 ...
 if "`power'"!="" {
   quietly tab `row' `col' , lrchi2 
   local potencia:disp "Power                `var_row_lab' vs `var_col_lab'= "  %5.3f 1-nchi2(  (r(c)-1)*(r(r)-1) ,r(chi2_lr),invchi2((r(c)-1)*(r(r)-1),1-0.05))
   local note="`note'"+"<p>"+"`potencia'"
 }
 ...
 forvalues i=1(1)`nrow' {
  ...
  if 	"`expected'"!="" {
   local variable= "   `variable'  <br> Expected <br> Adjusted residuals"
}   
 ...
 htput <TH ALIGN=CENTER COLSPAN=`ncol'> `var_col_lab' </TH>
   ...
   if `haytest'==1 {
       htput <TH VALIGN=BOTTOM ROWSPAN=2 > <p> p-value </p> <p> `method' </p> </TH>
   }
 ...
 htput <TR>
  foreach c in `collevels' {
	...
	if "`col'"=="total" local group=""
	if "`c'"=="." local group "Missing"
	htput <TH> `group' </TH>
	   }
 ...
 htput <TR> 		
  ...
  htput <TD ALIGN=left >  Total </TD>
   ...
   local j=0
    foreach c in `collevels' {
	  local j=`j' +1
	   htput  <TD ALIGN=center >  `cell_nrow_`j'' </TD>
	 }
	 if `total_fila' ==1  {
       htput <TD ALIGN=center  >   `n_total' (100%)  </TD>
     }
