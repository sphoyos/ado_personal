program define ht_tablacont, rclass
...
htput <TD  ALIGN= left  `coloret'>  `var_row_lab'  </TD>
   ...
   if `haytest_aov' ==1   {
		   ...
		   if `pnorm' >=0.05 {
        htsupput <font size=small > Test paramétrico mas adecuado : Prueba normalidad Shapiro_Wilk para `var_row_lab' p=`pvnorm'  </font size>
         }
  ...
  if `pnorm' <0.05 {
        local pvnorm: di %5.3f `pnorm'
         htsupput <font size=small > Test no paramétrico  mas adecuado: Prueba normalidad Shapiro_Wilk para `var_row_lab' p=`pvnorm'  </font size>
         }
