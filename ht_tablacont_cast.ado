program define ht_tablacont_cast, rclass
*! version 1.0 July 20011 de SPH en castella
...
*/                                      [MIssing] [minmax] [etifila(string)] [Format]  [psig(real 0.05)]/*
*/					[color]
...
/* CALCULA ELS PERCENTATGES PER COLUMNA SI SE DEMANEN ELS TOTALS PER COLUMNA */ 
 ...
 if "`uman'"!="" {
  ...
  if r(nx1)==.  local method_uman Mann-Whitney unilateral
...
if "`manexact'"!="" {
 ...
 if "`color'"!=""  {
  ...
  if "`color'"!=""  {
     ...
     local variable="N "
     if "`nomedia'" == "" {
              local variable: di "`variable'" "<p> Media (sd) </p>    		
                } 
 ...
 htput <TH ALIGN=CENTER COLSPAN=`ncol'> `var_col_lab' </TH>
   ...
   if `haytest_skil'==1 {
    ...
    if `haytest_uman'==1 {
       htput <TH VALIGN=BOTTOM ROWSPAN=2 > <p> p-valor </p> <p> `method_uman' </p> </TH>
   }
      if `haytest_manexact'==1 {
       htput <TH VALIGN=BOTTOM ROWSPAN=2 > <p> p-valor </p> <p> `method_manexact' </p> </TH>
   }
...
htput <TD  ALIGN= left  `coloret'>  `var_row_lab'  </TD>
   ...
   if `haytest_aov' ==1   {
		   ...
		   if "`swilk'"!="" {
		     if `pnorm' >=`psig'  {
               htsupput Mejor test: Test de normalidad de Shapiro_Wilk para  `var_row_lab' p=`pvnorm'
             }
		   }	 
