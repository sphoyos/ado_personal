...
program define htdoepi
	syntax,  chapter(string) item(string) [Hosp(string)]  [Com(string)] [Ghosp(string)]  [ecdc]
version 11.2
	clear
	...
	*/
	...
	if  length("`chapter'")!=3 {
		noi di in red "El número de chapter debe tener 3 digitos"
		error(1)
	}	
	
	
	if  length("`item'")!=2 {
		noi di in red "El número de itemp debe tener 2 digitos"
		error(1)
	}	
	/*  GENERA L'ESTRUCTURA DE DIRECTORIS SEGONS SIGA HOSPITAL, COMUNITAT O GLOBAL */
	/*  GENERA VARIABLES GLOBALS DE DIRECTORI/$dir_res , Nº CENTRO $sdir           */
   ...
   quietly do $labels\global_labels.do
	...
	if r(N)==0{
	noi di in red "El hospital no existe. Verifica el número"
		error(1)
	}
	else
	{
	...
	if r(N)==0{
	noi di in red "La comunidad no existe. Verifica el número"
		error(1)
	}
	else
	{
	...
	if r(N)==0{
	noi di in red "El grupo hospitalario no existe. Verifica el número"
		error(1)
	}
	else
	{
	...
	global dir_res= "$htm/Epine_Epps"
	local hosp="Epine_ECDC"
    global sdir="`hosp'"
	global description "EPINE ECDC"
	global tipoana="total"
	global tipoan="total"
	global if_sele "if hospitalid!=591 & ecdc==1"
	}
	 }
