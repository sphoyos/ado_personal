...
if "`mode'" == "long" {
	...
	while `i' <= _N {
		if "`noobs'" == "" {
			noi htput <TR>
			noi htdi "<TD colspan=`cols'> Observation " `: di `n'[`i']'  "</TD>"
			noi htput </TR>
		}
		else{
			if `i' != 1 {
		...
		while "`1'" != "" {
			...
			if substr("`type'",1,3) == "str" {
				...
				if "`y'" != "`x'" & "`nolabel'" == "" {
					...
					local celda: di  `format' `x'
				noi htput  <TD `valal'> `celda'"</TD>
				}
			}
			noi htput </TR>
			mac shift
