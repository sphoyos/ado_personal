...
local deplab: variable label `dep'
 if   `"`deplab'"' == "" local deplab = "`dep'"
  if "`multi'" == ""  {
    local analisi "Univariate regression `reg' per a `deplab' <br> " 
  }
  else {
     local analisi "Multivariate regression  `reg' per a `deplab' <br> " 
  }
}
...
*local opt= "`opt'" + " nohr"
  if "`multi'" == ""  {
   local analisi "Univariate regression `reg' " 
  }
  else {
    local analisi "Multivariate regression  `reg' " 
  }
}
...
if "`multi'"!="" {
	*  quietly
    ...
    local hc: disp  %8.4f r(C)
    	if "`reg'"=="streg" {
		*cap spvHarrellC
		cap estatC
		 local hc: disp  %8.4f r(HarrellC)
		}
	if "`reg'"=="stcox" & `hc'==. {
	     * cap spvHarrellC
		  cap estatC
	...
	if "`reg'"=="stcox"   {
		...
		htput <b>  Harell  C = `hc' </b> ; AIC model= `aic_model' ; BIC model= `bic_model' <BR> 
    }
  	...
  	if "`reg'"=="logit" {
		...
		htput <b>  R2 = `R2_model' ; Brierscore= `brierscore' </b> ; AIC model= `aic_model' ; BIC model= `bic_model'  <BR> 
    }
...
if "`options'"!="" {
     htput Options= "`options'"  <BR>   
	 if "`weib'"=="1"  {
	      streg
	      local sigma:disp %6.4f 1/ e(aux_p) 
		  local intercepto: disp  %6.4f _b[_cons]
	     htput  Sigma =  <b>`sigma' </b> ; Intercept =  <b>`intercepto' </b> <br>
		 htput <b>  Harell  C = `hc'  </b>; AIC model= `aic_model' ; BIC model= `bic_model'  <BR> 
	  }	 
   }
