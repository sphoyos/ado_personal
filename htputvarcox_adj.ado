...
program define htputvarcox_adj
version 9.1
syntax [if] [in], reg(string) [dep(varname)] indep(string) [rr(string)] [trans(string)] [per(string)] [multi] [PERMute(integer 0)] [color] [options(string)] [adjust(string)] [note(string)] [padj(real 1 )] 
...
local deplab: variable label `dep'
 if   `"`deplab'"' == "" local deplab = "`dep'"
  if "`multi'" == ""  {
    local analisi "Regressio  univariant `reg' per a `deplab'" 
  }
  else {
     local analisi "Regressio  Multivariant `reg' per a `deplab'" 
  }
}
...
local opt= "`opt'" + " nohr"
  if "`multi'" == ""  {
   local analisi "Regressio univariant `reg'. p valors ajustats " 
  }
  else {
    local analisi "Regressio multivariant `reg'. p valors ajustats " 
  }
}
...
if "`1'"!="" {
	...
	* permute before model estimation
	if `permute' != 0 {
	 ...
	 version 11.1: permute `dep' `v1'==r(p_`v1'), reps(`permute') seed(20080416) left: `_mymodel' `if' `in', cmd(`reg') dep(`dep') indep(`1') opt(`options') `adj'
	matrix p = r(p)
	matrix ci = r(ci)
	}
	...
	*noi di  in yellow %5.2f _se[``i'']
		...
		quietly testparm `1'
		if `padj'!=-1 {
		    local p=min(r(p)*`padj',1)
		}
		else{
		   local p=r(p)
		   local p=1-3.13* `p' *(1+1.65*ln(`p'))
		   }
	noi	disp in yellow "`1'"
