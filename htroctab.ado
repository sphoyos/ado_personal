...
prog def htroctab, rclass sortpreserve
	version 7, missing
	if _caller() < 8 {
		roctab_7 `0'
		return add
...
prog def MYDETail
	...
	noi htdi 
	noi htdi in gr "Detailed report of Sensitivity and Specificity"
	noi htdi in smcl in gr "{hline 78}"
	noi htdi in gr  _col(43) " Correctly"
	noi htdi in gr "Cutpoint      " "Sensitivity   Specificity" /*
	*/ "   Classified          LR+          LR-"
	noi htdi in smcl in gr "{hline 78}"
	qui label list `C'
	local max = r(k)
