program define htxtmixed_eng, rclass
...
syntax varlist(min=1 max=1 numeric) [if] [,adjust(string) logscale indeplog(string)] [batch(string)] id(string) [overall intraclass] [rr(string)] [pvaltest(string)] [options(string)]  
...
local lab_residual Residual
	...
	forvalues i=1(1)`= `nfix' ' {
		...
		if `i'==`nfix' local varlab "                               Intercept"
