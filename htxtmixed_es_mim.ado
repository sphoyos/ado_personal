program define htxtmixed_es_mim, rclass
...
version 10.1: 
xi: mim, storebv: xtmixed `dep' `adjust' `modlevels' , mle iterate(40)
esti store full
...
local lab_residual Residual
			...
			if substr("`1'",1,2) == "i." { /* categorical */
				...
				if "`pvaltest'" == "wald" {
					...
					if `lev_`vname'' > 2 {
						...
						xi:   `dep' `dropvar' `modlevels' if `sample' == 1, mle
						esti store nested
						lrtest full nested
						local op = r(p)
						post `op_exp' ("`1'") (`op')
	...
	forvalues i=`= `nfix' + 1'(1)`= `nfix' + `nrnd'' {
		...
		if "`varlab'" != "Residual"  local between "Entre-"
		else local between ""
	    htput <TR ALIGN="center">
		htput <TD COLSPAN="2"> `between'`varlab' Desv. Tipica. (<i>Err. Estand.</i>)</TD>
